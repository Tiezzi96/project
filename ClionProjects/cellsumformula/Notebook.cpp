...
Notebook::Notebook(const wxString &title) :
    ...
    //wxGrid * grid = new wxGrid(nb, wxID_ANY);
...
float Notebook::newcell() {
    ...
    //Cell *cell = new Cell(static_cast <float> (rand()) / static_cast <float> (RAND_MAX / 10));
...
void Notebook::deleteCell(wxCommandEvent &WXUNUSED(event)) throw(NumberCellsUnderflowException) {
    ...
    int i = rand() % 10;
    int j = rand() % 10;
    wxString s =(grid.front())->GetCellValue(i,j);
    while (s == "Null") {
        i = rand() % 10;
        j = rand() % 10;
        s = (grid.front())->GetCellValue(i, j);
        }

    double value;
    s.ToDouble(&value);
    float r = (float) value;
    float troncato=((float)((int)(r*1000.0f)))/1000.0f;


    float r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX / 10);
    float troncato=((float)((int)(r*100.0f)))/100.0f;
    for (auto itr2 = begin(cells); itr2 != end(cells); itr2++){
        if (troncato == (*itr2)->getValue()){
            cells.remove(*itr2);
            Sum->removeCell(*itr2);
            for(auto itr = begin(grid); itr != end(grid); itr++){
                (*itr)->SetCellValue(i,j,wxT("Null"));
            }
        }
    }*/
...
void Notebook::cellscontrol(wxCommandEvent& WXUNUSED(event)){
    ...
    for(auto itr2 = begin(c); itr2 != end(c); itr2++){
...
/*
void Notebook::SetCell(Cell *c) {

    wxString my_string = wxString::Format(wxT("%f"),c->getValue());
    int i= rand() % 10;
    int j=rand() %10;
    for(auto itr=begin(grid); itr!=end(grid); itr++){
        (*itr)->SetCellValue(i,j,my_string);
    }
...
for(auto itr3=cells.begin();itr3!=cells.end();itr3++){
    //std::cout<< (*itr3)->getValue()<<std::endl;
